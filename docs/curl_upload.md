...
Let's look at a quick test example on how to update/change the firmware section of station (given its 9-character id).
!!! WARNING 
...
```bash
curl -i -X PUT "https://gnss-metadata.eu/v1/sitelog/firmware-change?id=BRUX00BEL" -H  "accept: application/json" -H  "Authorization: Bearer xxx.." -H  "Content-Type: application/json" -d @firmware.json
```
...
{"id":"BRUX00BEL",
"uri":"https://gnss-metadata.eu/v1/sitelog/view?id=BRUX00BEL",
...
"_links":{"self":{"href":"https://gnss-metadata.eu/v1/sitelog/view?id=BRUX00BEL"},"geodesyML":{"href":"https://gnss-metadata.eu/v1/sitelog/exportxml?id=BRUX00BEL"},"sitelog":{"href":"https://gnss-metadata.eu/v1/sitelog/exportlog?id=BRUX00BEL"}}}
```
In the output information above, you also get the links to download the updated site log in various format e.g. https://gnss-metadata.eu/v1/sitelog/exportlog?id=BRUX00BEL
