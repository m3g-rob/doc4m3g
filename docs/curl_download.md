...
```bash
curl -s -X GET "https://gnss-metadata.eu/v1/sitelog/view?id=BRUX00BEL" -H  "accept: application/json"
```
...
```bash
curl -s -X GET "https://gnss-metadata.eu/v1/sitelog/view?id=BRUX00BEL" -H  "accept: application/json" | json_pp
```
...
```
{
   "_links" : {
      "geodesyML" : {
         "href" : "https://gnss-metadata.eu/v1/sitelog/exportxml?id=BRUX00BEL"
      },
      "self" : {
         "href" : "https://gnss-metadata.eu/v1/sitelog/view?id=BRUX00BEL"
      },
      "sitelog" : {
         "href" : "https://gnss-metadata.eu/v1/sitelog/exportlog?id=BRUX00BEL"
      }
   },
...
```bash
curl -s -X GET "https://gnss-metadata.eu/v1/sitelog/exportxml?id=BRUX00BEL" -H  "accept: application/geodesyML0.4"
```
```bash
curl -s -X GET "https://gnss-metadata.eu/v1/sitelog/exportlog?id=BRUX00BEL" -H  "accept: application/sitelog"
```
...
```bash
curl -g -s -X GET "https://gnss-metadata.eu/v1/sitelog?page=1&filter[id][like]=PRAG" -H  "accept: text/plain"
```
...
```Bash
curl -g -s -X GET "https://gnss-metadata.eu/v1/sitelog?page=1&filter[id][like]=PRAG" -H  "accept: text/plain" | awk '{print $1}'
```
and get:
```
id
...
```bash
curl -s -X GET "https://gnss-metadata.eu/v1/network/view?id=EPN" -H  "accept: application/json" | python -c "import sys,json; print(json.load(sys.stdin)['included'])"
```
...
```bash
curl -s -X GET "https://gnss-metadata.eu/v1/network/view?id=EPOS" -H  "accept: application/json" | python -c "import sys,json; print(json.load(sys.stdin)['proposed'])"
```
## `GET` list of satellite systems that a given station is currently tracking<a id='list_sat'></a>

```bash
curl -s -X GET "https://gnss-metadata.eu/v1/sitelog/view?id=BRUX00BEL" -H  "accept: application/json" | python -c "import sys,json; print(json.load(sys.stdin)['sitelog']['receivers'][-1]['satelliteSystem'])"
```
```
